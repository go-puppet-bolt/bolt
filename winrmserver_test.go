...
func (f *fakeWSMan) handle(w http.ResponseWriter, r *http.Request) {
	...
	case action == actionSend:
		f.doSend(w, body)
	case action == actionReceive:
		f.doReceive(w, body)
	case action == actionSignal:
...
func (f *fakeWSMan) doReceive(w http.ResponseWriter, body []byte) {
	...
	if last {
		...
		fmt.Fprintf(&sb, `<rsp:CommandState State="%s/CommandState/Done"><rsp:ExitCode>%d</rsp:ExitCode></rsp:CommandState>`, nsShell, exit)
	} else {
		fmt.Fprintf(&sb, `<rsp:CommandState State="%s/CommandState/Running"/>`, nsShell)
	}
...
func (f *fakeWSMan) reply(w http.ResponseWriter, body string) {
	...
	fmt.Fprintf(w, `<s:Envelope xmlns:s="http://www.w3.org/2003/05/soap-envelope" xmlns:rsp="%s"><s:Body>%s</s:Body></s:Envelope>`, nsShell, body)
}
